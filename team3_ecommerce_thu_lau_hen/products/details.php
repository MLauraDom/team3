...
if (isset($_GET["id"])) {
    ...
    if (isset($_SESSION['adm'])) {
        ...
        $delBtn = " <form method='POST' action='cart.php' class='m-auto mb-3'>
                        <div class='btn-wrapper-add m-auto w-75 text-center'>
                            <label for='quantity'>Quantity</label>
                            <input class='form-control form-control-sm w-25 m-auto' type='number' name='quantity' value='1' placeholder='1'>
                            <input type='hidden' name='product_id' value='{$product_id}'>
                            <br>
                            <input class='form-control form-control-sm btn btn-outline-dark text-center' type='submit' value='Add to Cart'>
                        </div>
        </form>";
    }
    ...
    if (mysqli_num_rows($query) == 1) {
        foreach ($result as $row) {
            $display = "
                <div class='card shadow mt-3 mb-5 mx-auto' style='width: 24rem; background-image: linear-gradient(to left, rgba(170, 163, 189, 0.7), rgba(120, 163, 209, 0.6));'>
...
<body
        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarCollapse">
                ...
                <div class="d-flex ms-5 justify-content-between py-2">
                    ...
                    <button class="btn btn-outline-primary dropdown-toggle mx-1 py-0" type="button"
                        data-bs-toggle="dropdown" aria-expanded="false"><i
                            class="bi bi-person-fill"></i></small></button>
                    <ul class="dropdown-menu dropdown-menu-end bg-muted">
                        ...
                        <p class="text-end pt-1 pb-0 mb-0">
                            ...
                            <a class="text-decoration-none my-0 mx-1 pt-2 pb-0"
                                href="<?= $logout_url ?>logout.php?logout"><sup>LogOut <i
                                        class="bi bi-box-arrow-right fw-bolder"></i></sup>
                            </a>
                            <!-- img and links -->
                        <div class="card-body text-center mb-0">
                            <div>
                                <img id="profile_pic" class="img-thumbnail rounded-circle" src="../pictures/<?= $pic ?>"
                                    alt="profile pic" style="width: 4rem; height: 4rem; object-fit: cover;">
                            </div>
                            <!-- full name -->
                            <p class="m-0 fw-bold">
                                <small><?= $fName . " " . $lName ?></small><br>
                                <sup><?= (@$_SESSION["adm"]) ? "<span class='text-danger fw-lighter'>(Admin)<span>" : "<span class='text-muted'>(User)</span>"; ?></sup>
                            </p>
                            <!-- email -->
                            <p class="m-0"><sup><?= $eMail ?></sup></p>
                            <hr class="mt-1 mb-0 py-1 bg-primary">
                        </div>
                        <!-- links -->
                        <li>
                            <a class="dropdown-item mt-0" href="cart.php"><small>My Cart</small></a>
                        </li>
                        <li>
                            <a class="dropdown-item" href="../user_update.php?id=<?= $user_id; ?>"><small>Edit
                                    Profile</small></a>
                        </li>
                        <li>
                        ...
                        <li>
                            <a class="dropdown-item text-decoration-none my-0 mx-1 py-0"
                                href="../logout.php?logout"><small>LogOut</small></a>
                        </li>
                    </ul>
    ...
    <!-- -----------------------------------------------RATINGS--------------------- -->
    <!-- RATINGS AND QUESTION BUTTONS -->
    <div class="q-btn-wrapper d-flex justify-content-evenly mb-3">
        <a class="btn btn-info p-2 border rounded shadow-lg" data-bs-toggle="collapse" href="#multiCollapseExample1"
            role="button" aria-expanded="false" aria-controls="multiCollapseExample1"><small>Ratings &
                Reviews</small></a>
        <a class="btn btn-primary p-2 border rounded shadow-lg" type="button" data-bs-toggle="collapse"
            data-bs-target="#multiCollapseExample2" aria-expanded="false" aria-controls="multiCollapseExample2"
            href="#ques"><small>Questions from our
                users</small>
        </a>
    </div>

    <!-- Questions and Ratings Container -->
    <div class="container d-flex justify-content-evenly align-items-top m-auto text-center shadow-lg"
        style="width:92%;">


        <div class="collapse multi-collapse" id="multiCollapseExample1">
            <div class="card card-body mb-4"
                style="background-image: linear-gradient(to top, rgba(200, 133, 189, 0.7), rgba(120, 133, 189, 0.7)); background-repeat: no-repeat;">

                <section class="mx-auto w-100 <?= (@($_SESSION['adm'])) ? 'd-none' : ''; ?>">
                    <h3 class="mb-3">Leave us a review:</h3>
                    <form method="POST" class="form-control shadow mb-5 py-3 bg-muted" action="actions/a_review.php">
                        ...
                        <input class="form-range my-1 py-0" name="rating" type="range" id="rating" min="1" max="5">
                ...
                <p class="h4 text-light">Product Reviews & Ratings</p>
                <?php echo $comment ?>
            </div>
        </div>
        ...
        <div class="collapse multi-collapse" id="multiCollapseExample2" id="ques">
            <div class="card card-body m-auto w-100 mb-4"
                style="background-image: linear-gradient(to bottom, rgba(200, 133, 189, 0.7), rgba(120, 133, 189, 0.7)); background-repeat: no-repeat;">
                <section class="mx-auto w-100 <?= (@($_SESSION['adm'])) ? 'd-none' : ''; ?>">
                    <h3 class="mb-3 text-light">Do you have a Question about this product?</h3>

                    <form method="POST" class="form-control shadow m-auto mb-5 py-3 bg-muted"
